...
$F2::{
	try{
		if ImageSearch(&FoundX, &FoundY, 900, 550, 1100, 850, "*TransBlack *4 " A_ScriptDir "\images\yes_prota.png"){
			msgbox "`"Yes`" option not selected. `n`nIf this isn't supposed to be the case, your tolerance parameter (the number after the * before A_ScriptDir, default is 4) is too high. `nModify this parameter before trying again."
		}
		else msgbox "`"Yes`" option selected. `n`nIf this isn't supposed to be the case, you can try modifying the tolerance parameter (the number after the * before A_ScriptDir, default is 4) between 0 and 255 before trying again."
	}
}

$F3::{
	try{
		if ImageSearch(&FoundX, &FoundY, 435, 760, 465, 790, "*TransBlack *12 " A_ScriptDir "\images\repeat_prota.png"){
...
$F4::{
	try{
		if ImageSearch(&FoundX, &FoundY, 530, 170, 570, 210, "*TransBlack *8 " A_ScriptDir "\images\coin_prota.png"){
