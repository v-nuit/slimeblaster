...
$F3::{
	try{
		if ImageSearch(&FoundX, &FoundY, 435, 760, 465, 790, "*TransBlack *8 " A_ScriptDir "\images\repeat_nb.png"){
			msgbox "Repeat marker detected. `n`nIf this isn't supposed to be the case, your tolerance parameter (the number after the * before A_ScriptDir, default is 8) is too high. `nModify this parameter before trying again."
		}
		else msgbox "Repeat marker not detected. `n`nIf this isn't supposed to be the case, you can try modifying the tolerance parameter (the number after the * before A_ScriptDir, default is 8) between 0 and 255 before trying again."
	}
}

$F4::{
	try{
		if ImageSearch(&FoundX, &FoundY, 530, 170, 570, 210, "*TransBlack *8 " A_ScriptDir "\images\coin_nb.png"){
